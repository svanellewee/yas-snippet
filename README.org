...
* 📸 yas-snippet
**Yet Another Screenshot Snippet** – A lightweight, cross-platform screenshot annotation tool designed specifically for *macOS* and *Sway (Wayland)*.
...
#+BEGIN_SRC nix
inputs.yas-snippet.url = "github:svanellewee/yas-snippet";
#+END_SRC
...
{ inputs, pkgs, ... }: {
  home.packages = [
    inputs.yas-snippet.packages.${pkgs.system}.default
  ];
}
#+END_SRC
